...
import { Check, Plus, ArrowRight, Bot, ShieldCheck, Code2 } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({ item, onSelect }) => {
  return (
    <article className="relative bg-[#060c16] border border-slate-800/90 rounded-[28px] overflow-hidden shadow-2xl flex flex-col hover:border-cyan-500/40 hover:shadow-cyan-950/20 transition-all duration-300 group">
      ...
      {/* ========================================================
          CABECERA SUPERIOR CLARA (LOGO + BADGES)
      ======================================================== */}
      <div className="relative bg-gradient-to-b from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] p-4 sm:p-5 rounded-t-[26px] flex flex-col justify-between h-44 sm:h-48 overflow-hidden select-none">
        
        {/* Fila superior de Badges */}
        <div className="flex items-center justify-between z-10">
          {item.badgeLeft ? (
            <span className="bg-[#202738] text-slate-200 text-[10px] font-bold tracking-wider px-3.5 py-1 rounded-full uppercase shadow-sm">
              {item.badgeLeft}
            </span>
          ) : <div />}

          {item.badgeRight && (
            <span className="bg-[#10b981] text-[#022c22] text-[10px] font-extrabold tracking-wider px-3.5 py-1 rounded-full uppercase shadow-sm">
              {item.badgeRight}
            </span>
          )}
        </div>
        ...
        {/* Icono flotante inferior izquierdo en la cabecera */}
          ...
          <div className="flex items-end justify-between gap-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                className="w-8 h-8 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white hover:border-slate-600 flex items-center justify-center transition-all hover:scale-105"
